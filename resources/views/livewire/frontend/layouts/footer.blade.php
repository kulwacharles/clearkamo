                            ...
                            <div class="menu-all-pages-container">
                                <ul class="menu">
                                    @foreach ($services as $service)
                                        <li><a wire:navigate href="/service/details/{{ $service->slug }}">{{ $service->title }}</a></li>                                
                                    @endforeach
                                ...
                                @if($blogs)
                                    @foreach ($blogs as $blog)
                                        <div class="recent-post">
                                            <div class="media-img">
                                                <a wire:navigate href="/news-and-updates/details/{{ $blog->slug }}"><img src="{{ asset('storage/'.$blog->image) }}" alt="Blog Image"></a>
                                            </div>
                                            <div class="media-body">
                                                <div class="recent-post-meta">
                                                    <a wire:navigate href="/news-and-updates/details/{{ $blog->slug }}"><i class="fal fa-calendar-days"></i>{{$blog->updated_at->format('d F, Y')}}</a>
                                                </div>
                                                <h4 class="post-title"><a class="text-inherit" wire:navigate href="/news-and-updates/details/{{ $blog->slug }}">{{$blog->title}}</a></h4>
                                            </div>
                                        </div>
                                     @endforeach
                                @endif
